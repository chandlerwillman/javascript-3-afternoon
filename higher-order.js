...
let orderTotals = orders.map((val,i,arr) => arr[i]["price"] * (1 + arr[i]["tax"]));
...
let bobsOrder = purchases.filter((val,i,arr) => arr[i]["owner"] == "Bob");
let bobsTotal = bobsOrder.reduce((total,val,i,arr) => {return total += arr[i]["price"]}, 0);
