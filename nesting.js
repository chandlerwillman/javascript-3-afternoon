...
function employeeUpdater(){
  ...
  for(let i = 0; i < arrCopy.length; i++){
    if(arrCopy[i]["firstName"] == "Theo"){
      employees.splice(i,1);
    }else if(arrCopy[i]["firstName"] == "Lorie"){
...
removeDuplicates = function removeDuplicates(){
  let arrCopy = [workplaceAccidents[0]];
  for(let i = 1; i < workplaceAccidents.length; i++){
      if(!arrCopy.includes(workplaceAccidents[i])){
          arrCopy.push(workplaceAccidents[i]);
      }
  }
  return arrCopy;
}
